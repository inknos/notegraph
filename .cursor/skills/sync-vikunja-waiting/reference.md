...
- Tasks with a `start_date` automatically get a **`due_date`** of **start + 7 days**. Tasks without a `start_date` (not assigned to you) get **no** `due_date`. Both dates are included in the diff comparison and updated on subsequent syncs.
